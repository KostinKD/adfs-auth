...
onDevtoolsClientConnected(async (client: NuxtDevtoolsIframeClient) => {
  ...
  oidcState.value = devtoolsClient.value.host.nuxt.payload.state['$sadfs-auth-session'] || {}
  oidcSecrets.value = isDevAuthed.value ? await devtoolsClient.value.devtools.extendClientRpc('adfs-auth-rpc').getNuxtOidcAuthSecrets() : {}
})
