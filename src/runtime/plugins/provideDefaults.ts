...
export default defineNitroPlugin(async () => {
  if (!process.env.NUXT_OIDC_SESSION_SECRET || process.env.NUXT_OIDC_SESSION_SECRET.length < 48) {
    ...
    console.warn('[adfs-auth]: No session secret set, using a random secret. Please set NUXT_OIDC_SESSION_SECRET in your environment with at least 48 chars.')
    console.info(`[adfs-auth]: NUXT_OIDC_SESSION_SECRET=${randomSecret}`)
  }
  if (!process.env.NUXT_OIDC_TOKEN_KEY) {
    ...
    console.warn('[adfs-auth]: No refresh token key set, using a random key. Please set NUXT_OIDC_TOKEN_KEY in your environment. Refresh tokens saved in this session will be inaccessible after a server restart.')
    console.info(`[adfs-auth]: NUXT_OIDC_TOKEN_KEY=${randomKey}`)
  }
  if (!process.env.NUXT_OIDC_AUTH_SESSION_SECRET) {
    ...
    console.warn('[adfs-auth]: No auth session secret set, using a random secret. Please set NUXT_OIDC_AUTH_SESSION_SECRET in your environment.')
    console.info(`[adfs-auth]: NUXT_OIDC_AUTH_SESSION_SECRET=${randomKey}`)
  }
})
